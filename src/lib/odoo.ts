/**
 * Build-time Odoo fetcher for Digital Stackers blog posts.
 * Queries blog.post in portal.way4tech.com filtered by DS_BLOG_ID
 * (default = 2, the "Digital Stackers" blog.blog record).
 *
 * Env vars (both local .env and GitHub Actions):
 *   ODOO_URL       https://portal.way4tech.com
 *   ODOO_DB        portal.way4tech.com
 *   ODOO_USER      website_api
 *   ODOO_PASSWORD  <secret>
 *   DS_BLOG_ID     2
 *
 * On any failure (env missing, network, auth), returns [] so the build
 * doesn't crash — MDX posts will still ship.
 */
...
async function rpc(url: string, path: string, params: unknown, cookie?: string): Promise<{ result: any; cookie?: string }> {
  ...
  if (data.error) throw new Error(`Odoo ${path}: ${data.error.data?.message || data.error.message}`);
...
export async function fetchOdooBlogPosts(): Promise<OdooBlogPost[]> {
  ...
  const blogId = parseInt(process.env.DS_BLOG_ID || '2', 10);
  ...
  try {
    ...
    const posts = await rpc(
      ...
      '/web/dataset/call_kw',
      {
        ...
        method: 'search_read',
        args: [
          [['website_published', '=', true], ['blog_id', '=', blogId]],
    ...
    return raw.map(p => {
      ...
      const author = Array.isArray(p.author_id) ? p.author_id[1] : 'Digital Stackers';
