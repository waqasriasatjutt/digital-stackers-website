/**
 * POST /api/lead
 * Accepts Digital Stackers contact / audit form submissions and pushes
 * them as crm.lead (type=opportunity) into the Odoo CRM at portal.way4tech.com.
 *
 * Runs at the Cloudflare edge — Odoo credentials never reach the browser.
 *
 ...
 *   LEAD_TEAM_ID   (optional) crm.team id for "Digital Stackers" pipeline
 *   LEAD_SALESPERSON_ID (optional) res.users id to route leads to
 */
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    const descLines = [
      `[Digital Stackers website]`,
      body.service ? `Service: ${body.service}` : '',
      body.budget  ? `Budget:  ${body.budget}`  : '',
      body.country ? `Country: ${body.country}` : '',
      body.source  ? `Source:  ${body.source}`  : '',
      body.website ? `URL:     ${body.website}` : '',
      '',
      message,
    ].filter(Boolean);

    const leadTitle = body.service
      ? `DS · ${body.service} — ${body.company || name}`
      : `DS · Contact — ${body.company || name}`;
    ...
    const { data: leadId, cookie: c2 } = await odooCall(
      env,
      '/web/dataset/call_kw',
      { model: 'crm.lead', method: 'create', args: [vals], kwargs: {} },
      cookie,
    );

    // Chatter post so the message is visible right on the opportunity page.
    const chatter =
      `Digital Stackers website form submission\n` +
      `\n` +
      `Name: ${name}\n` +
